...
class Address {
  ...
  ///用户的仓库 get
  static userRepos({@required userName, sort = "pushed"}) {
  ...
  ///自己的star get
  static myStar({sort = "updated"}) {
    return "${host}users/starred?sort=$sort";
  }

  ///用户的star get
  static userStar({@required userName, sort = "updated"}) {
  ...
  ///仓库路径下的内容 get
  static reposDataDir({@required reposOwner, @required repos, @required path, branch = "master"}) {
    return "${host}repos/$reposOwner/$repos/contents/$path" + ("?ref=" + branch);
  }

  ///README 文件地址 get
  static readmeFile({reposNameFullName, curBranch = ""}) {
    return host + "repos/" + reposNameFullName + "/" + "readme" + ("?ref=" + curBranch);
  }
  ...
  ///fork get
  static getForker({reposOwner, reposName, sort = "newest"}) {
  ...
  ///通知 get
  static getNotification({all = false, participating = false}) {
